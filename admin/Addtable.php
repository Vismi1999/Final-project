      ...
      <div class="row">
       <!-- <p class="elementToFadeInAndOut"><?php if(isset($_SESSION['msg'])) {echo $_SESSION['msg'];} ?></p> -->
        <div class="column"> 
           ...
           <form  method="post" action="Addtable.php" name="tableseat" onsubmit="return Register()" enctype="multipart/form-data">
           <label for="last_name" >Add Table Size</label>
          
            <input type="text" id="status" name="tableseat"onclick="validate();" value="" autocomplete="off" placeholder="Enter seat size" />
            <label for="last_name" >Total Table</label>
          
            <input type="text" id="status" name="totaltable"onclick="validate();" value="" autocomplete="off" placeholder="Total Table"/>
...
if(isset($_POST['submit'])){
  $table = $_POST['tableseat'];
  $totaltable = $_POST['totaltable'];
    ...
    $query = "SELECT * FROM `tbl_table` Where tableseat='$table'";
$result = mysqli_query($conn,$query);

 if(mysqli_num_rows($result)>0)
 {
    // echo "item exits";
    echo "<script>alert('Tablesize Already Exist.');window.location.href='Addtable.php'</script>";

 }
else{
  $sql="insert into tbl_table(`tableseat`,`totaltable`,`status`) 
        VALUES('$table','$totaltable',',1')";
    ...
    // }
    if($result){
        echo "<script>alert('Table added successfully.');window.location.href='#'</script>";
       }else{
        echo "<script>alert('Table not added.');window.location.href='#'</script>";
        die(mysqli_error($conn));
       }
    }
}
